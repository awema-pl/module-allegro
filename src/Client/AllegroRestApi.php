...
class AllegroRestApi
{
    ...
    {
        $url = (!$sandbox) ? 'https://allegro.pl/auth/oauth/authorize' : 'https://allegro.pl.allegrosandbox.pl/auth/oauth/authorize';
        return $url
            . "?response_type=code"
            . "&client_id=$clientId"
    ...
    public static function generateToken($code, $clientId, $clientSecret, $redirectUri, $sandbox = false)
    {
        ...
        $url = (!$sandbox) ? 'https://allegro.pl/auth/oauth/token' : 'https://allegro.pl.allegrosandbox.pl/auth/oauth/token';
        // Returning response
    ...
    public function sendRequest($resource, $method, $data = array(), array $headers = array(), $json = true)
    {
        ...
        // We have found an error in response for OAuth2
        if (isset($response->error) || isset($response->error_description)) {
            throw new AllegroApiException(
                'Allegro API error.', AllegroApiException::ERROR_API_ALLEGRO, (int)$this->getResponseCode($http_response_header),
                null, null, sprintf('%s. %s.', $response->error, $response->error_description)
            );
            // We have found an error in response for Allegro API
        }
        else if (isset($response->errors)) {
            $error = ($response->errors[0] ?? null) ? $response->errors[0] : null;
            $errorUserMessage = ($error) ? $error->userMessage : null ;
            throw new AllegroApiException(
                'Allegro API error.', AllegroApiException::ERROR_API_ALLEGRO, (int)$this->getResponseCode($http_response_header),
                null, $errorUserMessage, sprintf('%s', json_encode($response->errors, JSON_UNESCAPED_UNICODE))
            );
        }
